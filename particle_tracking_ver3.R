...
tracks_calc <- function (a, b){
  c <- within(merge(a,b,by="TRACK_ID"), {
    ...
    ANGLE_CAT<-cut(ANGLE, seq(0,180,4))
    VELOCITY <- VELOCITY.x
    ...
    ANGLE_SINGLE_CAT <-cut(ANGLE_SINGLE, seq(0,180,4))
...
tracks_summary$ANGLE_RAD <- atan2(tracks_summary$DISP_Y, tracks_summary$DISP_X)
tracks_summary$ANGLE_RAD_2 <- NISTdegTOradian(tracks_summary$ANGLE)
...
tracks_summary$X_SINGLE <- ave(tracks_summary$X_LOCATION, tracks_summary$TRACK_ID,
                               FUN=function(x) c(0, diff(x)))
tracks_summary$Y_SINGLE <- ave(tracks_summary$Y_LOCATION, tracks_summary$TRACK_ID,
                               FUN=function(x) c(0, diff(x)))
tracks_summary$DISP_2 <- sqrt((tracks_summary$X_SINGLE)^2 + 
                                (tracks_summary$Y_SINGLE)^2)
tracks_summary$TIME_SINGLE <- ave(tracks_summary$TIME, tracks_summary$TRACK_ID,
                                  FUN=function(x) c(0, diff(x)))
tracks_summary$ANGLE_SINGLE <- sapply((atan2(tracks_summary$Y_SINGLE, 
                                             tracks_summary$X_SINGLE)*180)/pi, pos_angles)
tracks_summary$ANGLE_SINGLE_CAT <-cut(tracks_summary$ANGLE_SINGLE, seq(0,180,4))
tracks_summary$VELOCITY_CALC <- tracks_summary$DISP_2/tracks_summary$TIME_SINGLE
